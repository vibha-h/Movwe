            ...
            : Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      int lobbyId = int.tryParse(_lobbyIdController.text) ?? 0;
                      if (lobbyId != 0) {
                        // Call the joinLobby function from LobbyViewModel
                        bool success = await lobbyViewModel.joinLobby(context, lobbyId);

                        // Show success or failure message based on the result
                        if (success) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("Successfully joined the lobby!")),
                          );
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("Error joining the lobby.")),
                          );
                        }
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text("Please enter a valid lobby ID.")),
                        );
                      }
                    },
