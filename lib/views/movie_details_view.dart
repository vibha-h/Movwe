      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                final lobbyId = lobbyViewModel.joinCode;
                if (lobbyId != null) {
                  final lobby = lobbyViewModel.getLobby(lobbyId);
                  if (lobby != null) {
                    // Add movie ID to the lobby's movieIds list
                    lobbyViewModel.addMovie(movie as String);
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content: Text("Failed to add movie to lobby.")),
                    );
                  }
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("No lobby joined.")),
                  );
                }
              },
              child: const Text('Add Movie to Lobby'),
            ),
          ],
        ),
      ),
